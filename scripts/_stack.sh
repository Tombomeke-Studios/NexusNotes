...
# the backend this script starts is torn down on exit. If a backend is already
# healthy on :8080 it is reused instead of started again.
set -euo pipefail
...
start_stack() {
  ...
  if curl -sf "http://localhost:$PORT/health" >/dev/null 2>&1; then
    log "Backend already healthy on :$PORT — reusing it."
    return
  fi
